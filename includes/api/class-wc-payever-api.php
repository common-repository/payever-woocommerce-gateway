...
if ( class_exists( 'WC_Payever_Api' ) ) {
	return;
}
...
class WC_Payever_Api {
	...
	public function get_payments_api_client() {
		if ( null === $this->payments_api_client ) {
			$this->payments_api_client = new PaymentsApiClient(
	...
	private function prepare_logger() {
		$this->client_configuration->setLogger( new WC_Payever_Logger() );
	...
	private function prepare_locker() {
		...
		if ( wp_mkdir_p( $payever_dir ) && wp_is_writable( $payever_dir ) ) {
