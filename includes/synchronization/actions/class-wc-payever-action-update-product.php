...
if ( class_exists( 'WC_Payever_Synchronization_Action_Handler_UpdateProduct' ) ) {
	return;
}
...
class WC_Payever_Synchronization_Action_Handler_UpdateProduct implements ActionHandlerInterface, LoggerAwareInterface {
	...
	private function import_product_variations( $product_id, $variants, $entity_currency ) {
		...
		foreach ( $variants as $variant ) {
			...
			$variation_id  = $this->get_helper()->get_product_variation_id_by_sku( $variation_sku );
			if ( ! $variation_id ) {
	...
	private function process_product_categories( WC_Product $wc_product, ProductRequestEntity $product_entity ) {
		...
		$this->save_product_categories( $wc_product, $product_category_ids );
	}
	 ...
	 * @SuppressWarnings(PHPMD.ElseExpression)
	 */
	private function save_product_categories( WC_Product $wc_product, array $product_category_ids ) {
	...
	private function process_product_import( ProductRequestEntity $product_entity ) {
		...
		if ( ! $post_id || ! $this->get_wp_wrapper()->wc_get_product( $post_id ) ) {
	...
	private function save_product_meta( $product_entity, $entity_currency, $product_id ) {
		...
		$meta_data  = array(
			...
			'_width'                 => '',
			'_height'                => '',
			'_sku'                   => $product_entity->getSku() ?: str_replace( ' ', '_', strtolower( (string) $product_entity->getTitle() ) ),
	...
	private function download_image( $local_path, $url ) {
		if ( is_file( $local_path ) && ! wp_is_writable( $local_path ) ) {
			return false;
		}

		if ( ! wp_is_writable( dirname( $local_path ) ) ) {
			return false;
		}
