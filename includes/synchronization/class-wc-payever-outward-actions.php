...
if ( class_exists( 'WC_Payever_Outward_Actions' ) ) {
	return;
}
...
class WC_Payever_Outward_Actions {
	...
	public function product_delete_after( $product_id ) {
		...
		$this->remove_product_request( $product );
	 ...
	 * @return bool
	 */
	private function remove_product_request( WC_Product $product ) {
		$this->get_logger()->debug( 'Handling woocommerce_delete/trash_product event' );

		$product_removed_request = new ProductRemovedRequestEntity();
		$product_removed_request->setSku( $product->get_sku() );
	...
	public function product_object_updated_props( $product, $updated_props ) {
		...
		if ( in_array( 'stock_quantity', $updated_props ) ) {
			$this->get_logger()->debug( 'Handling woocommerce_product_object_updated_props event' );
	...
	private function create_or_update_product( $product_id, $product, $is_new_product = null ) {
		...
		if ( in_array( $product->get_type(), array( 'simple', 'external', 'variable', 'downloadable', 'virtual' ) ) ) {
			...
			$product_request->setExternalId( $this->get_helper()->get_product_sync_token() );
	...
	private function is_action_valid() {
		if ( ! $this->get_helper()->is_products_sync_enabled()
